...
### 2026-05-03 — תמיכה בהקלטות יוטיוב לאזכרה (`youtube.txt`)
**הקשר:** האזכרה של 2020 התקיימה בזום בגלל הקורונה ויש הקלטה ב-MP4. בחרנו ב-YouTube Unlisted כפלטפורמה (חינם, סטרימינג מותאם, נגן יציב).
**קונבנציה חדשה:** קובץ `youtube.txt` בשורש תיקיית האזכרה (`site/imgs/memorials/dd_mm_yyyy/youtube.txt`). תוכן הקובץ הוא או Video ID 11-תוויי או URL מלא של YouTube — הסקריפט מזהה את שתי הצורות (כולל `youtu.be`, `youtube.com/watch?v=`, `youtube.com/embed/`, `youtube.com/shorts/`).
**מה שונה:**
- `scripts/build_memorials_manifest.py`: נוסף `extract_youtube_id()` ובלוק שקורא `youtube.txt` ושם `youtube: <id>` ב-manifest entry. הסיכום שמודפס בסוף מציג `YT=<id>` למי שיש לו.
- `site/memorial.html`: סקציה חדשה `#videoSection` עם iframe רספונסיבי 16:9. משתמש ב-`https://www.youtube-nocookie.com/embed/<id>` (מצב פרטיות מוגבר של גוגל). הסקציה מופיעה לפני הספדים ותמונות. ה-empty-state עכשיו לוקח בחשבון גם youtube — אזכרה עם רק וידאו לא תיחשב ריקה.
