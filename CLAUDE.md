...
### 2026-05-03 — הכהיית כל שלוש רמות הטקסט
**הקשר:** הכהיית `--text` בלבד הקודמת לא הספיקה כי רוב הפרוזה הנושאית באתר משתמשת ב-`--text2` (פסקאות סיפור חיים, מורשת קרב, גוף ההספדים, גוף האזכרות), ו-`--text3` משמש ל-labels/footer/כפתורי סינון. תיראן ביקש שכל השלוש יהיו כהים יותר.
**מה שונה (`site/index.html`, `site/memorial.html`):**
- `--text:  #d4cdb8 → #b8b09a` (זהו הצעד השני; הצעד הראשון היה מ-#f0ead8)
- `--text2: #a89e88 → #8b8270`
- `--text3: #6b6257 → #5d544a`
ההפרשים היחסיים בין השלוש נשמרו, כך שההיררכיה הוויזואלית (ראשי / משני / מאופק) נותרת זהה — רק כל הפלטה זזה לכיוון כהה.
**הערת נגישות:** `--text3` החדש על רקע שחור יורד לקונטרסט ~3.0:1 — מתחת ל-WCAG AA לטקסט רגיל, אבל הוא משמש רק לאלמנטי UI לא-קריטיים (footer, group labels, captions). אם תיראן יציין שמשהו מהדברים האלה לא נקרא — להעלות טיפה ל-`#675e54` (קונטרסט ~3.4:1).
**מה משתמש ב-מה (לעתיד)