...
function buildImgList() {
  ...
  // battle gallery
  document.querySelectorAll('#battleInner .gallery-item').forEach(function(el) {
    var img = el.querySelector('img');
    var cap = el.querySelector('.gallery-caption');
    var full = el.dataset.full || (img ? img.src : '');
    IMG_LIST.push({ src: full, caption: cap ? cap.textContent : '', group: 'battle' });
  });
}
...
window.battleScroll = function(){};

// ── Build pics + newspaper + battle galleries from data/pics.json ──
// (Manifest is auto-generated by scripts/build_pics_manifest.py — adding
//  files to imgs/pics or imgs/newspaper rebuilds it via GitHub Actions.)
function buildGalleryFromManifest(containerId, items, group) {
  ...
  items.forEach(function(item) {
    ...
    div.addEventListener('click', function() { openImg(item.full, caption, group); });
    var img = document.createElement('img');
    img.src = item.thumb || item.full;
    img.alt = caption;
    img.loading = 'lazy';
    img.onerror = function() { img.onerror = null; img.src = item.full; };
    div.appendChild(img);
    if (caption) {
...
fetch('data/pics.json')
  .then(function(r) { return r.json(); })
  .then(function(data) {
    buildGalleryFromManifest('galleryInner',    data.pics      || [], 'pics');
    buildGalleryFromManifest('newspaperInner',  data.newspaper || [], 'newspaper');
    buildGalleryFromManifest('battleInner',     data.battle    || [], 'battle');
    ...
    if (data.battle && data.battle.length) {
      document.getElementById('battleGallery').style.display = '';
      setupLazyCarousel(document.querySelector('#battle details.collapsible'), 'battleInner', 'battleDots', 'battleScroll');
    }
  })
  .catch(function(e) { console.warn('pics manifest load failed:', e); });
...
(function setupAzkarotGrid() {
  ...
  function counts(m) {
    if (!m) return '';
    var parts = [];
    var ytCount = Array.isArray(m.youtube) ? m.youtube.length : (m.youtube ? 1 : 0);
    if (ytCount > 0) parts.push(pluralize(ytCount, 'סרטון', 'סרטונים'));
